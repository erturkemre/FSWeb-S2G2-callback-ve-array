...
let mac = fifaData.filter((finalMac) => {
  return finalMac["Year"] === 2014 && finalMac["Stage"] === "Final";
});
console.log(mac[0]["Home Team Name"]);

//(b) 2014 Dünya kupası Finali Deplasman takım ismi  (dizide "Away Team Name" anahtarı)

console.log(mac[0]["Away Team Name"]);

//(c) 2014 Dünya kupası finali Ev sahibi takım golleri (dizide "Home Team Goals" anahtarı)
console.log(mac[0]["Home Team Goals"]);
//(d)2014 Dünya kupası finali Deplasman takım golleri  (dizide "Away Team Goals" anahtarı)
console.log(mac[0]["Away Team Goals"]);
//(e) 2014 Dünya kupası finali kazananı*/
if (mac[0]["Home Team Goals"] > mac[0]["Away Team Goals"]) {
  console.log("Kazanan: ", mac[0]["Home Team Name"]);
} else if (mac[0]["Home Team Goals"] < mac[0]["Away Team Goals"]) {
  console.log("Kazanan:", mac[0]["Away Team Name"]);
}
...
function Yillar(fifaData, Finaller) {
  let years = Finaller(fifaData).map((finalMac) => {
    return finalMac["Year"];
  });
...
function Kazananlar(fifaData, callback) {
  let kazananlar = callback(fifaData).map((kazanan) => {
    return kazanan["Home Team Goals"] > kazanan["Away Team Goals"]
      ? kazanan["Home Team Name"]
      : kazanan["Away Team Name"];
  });
  return kazananlar;
}
...
function YillaraGoreKazananlar(
  ...
  let yillar = cb_yillar(fifaData, cb_finaller);
  let kazananlar = cb_kazananlar(fifaData, cb_finaller);
  let yillarKazananlar = yillar.map((yil, index) => {
